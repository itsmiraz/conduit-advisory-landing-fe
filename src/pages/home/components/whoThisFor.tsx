    ...
    <div className="py-[31px] px-[20px] mb-[285px]">
      <div className="flex  flex-col justify-start md:justify-center  items-start md:items-center">
        ...
        <h2 className="text-[28px] md:text-[40px] py-[10px] md:text-center font-instrument leading-[120%] text-start font-medium">
          We work with forward-thinking teams across every <br  className="md:block hidden"/> industry and
          growth stage — to build, launch, and scale <br className="md:block hidden" />{" "}
          <span className="text-[#BB7AFF]"> something that matters.</span>
        </h2>
        {/* desktop view */}
        <div 
          style={{ backgroundImage: `url(${backGroundImage})` }} className="md:h-[590px]  grid-cols-1 md:grid hidden md:grid-cols-2  md:w-[1010px]  bg-no-repeat bg-cover bg-center ">
        ...
        <div 
          ...
          {audiences.slice(0,2).map((item, i) => (
            ...
            >
              ...
              <p className="text-[#E2E1E5] text-[14px] md:text-[16px] leading-[24px] font-instrument text-center">{item.description}</p>
            </div>
          ))}
