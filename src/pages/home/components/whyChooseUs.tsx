...
const WhyChooseUs = () => {
  return (
    <div className="pt-[150px] py-[132px] px-32 max-w-[1700px] mx-auto">
      <div>
        <div className="flex justify-between items-end">
          <div>
            ...
            <h2 className="text-[40px] pt-[18px] font-medium font-instrument">
              Because we're the only agency focused on <br />{" "}
              <span className="text-[#443D52]">alignment</span> — engineered
              through systems,
              <br /> powered by technology, and driven by <br /> growth.
            </h2>
          </div>
        ...
        <div className="featureCardContainer  ">
          {features.map((item, i) => (
            <div className="max-w-[380px] cursor-pointer p-[24px] featureCardBg rounded-[16px] border-[1px] border-[#FFFFFF14]" key={i}>
              <div>{item.icon}</div>
              <h2 className="text-[24px] leading-[36px] font-medium">{item.title}</h2>
              <h3 className="text-[18px] py-[10px]">{item.subtitle}</h3>
              <p className="pt-[72px] text-[16px] leading-[24px]">
                {item.description}
              </p>
            </div>
          ))}
